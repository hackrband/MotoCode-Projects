  ...
  useEffect(() => {
    ...
    const update = (time: number) => {
      ...
      if (eOn && !isHot) {
        ...
        // Detect Shift for visual dip
        if (curGear !== lastGearRef.current) {
          if (curGear > lastGearRef.current && lastGearRef.current !== 0) {
            shiftTimerRef.current = 0.15; // 150ms shift dip
          }
          lastGearRef.current = curGear;
        }

        if (isThrottling && shiftTimerRef.current <= 0) {
        ...
        if (!isClutching && curGear !== 0) {
          ...
          if (shiftTimerRef.current > 0) {
            // Apply a temporary dip during shift
            targetRpm = engineRpmFromSpeed * 0.8;
          } else {
        ...
        if (shiftTimerRef.current > 0) {
          shiftTimerRef.current -= dt;
        }

        setRpm(prev => {
          const diff = targetRpm - prev;
          let accel = isThrottling ? 15000 : 5000;
          
          // Higher gears accelerate slower if clutch is out
          if (curGear !== 0 && !isClutching && isThrottling) {
            // GEAR_RATIOS[1] is 5.0, GEAR_RATIOS[5] is 2.0
            // Scale acceleration based on mechanical advantage
            const gearFactor = GEAR_RATIOS[curGear] / GEAR_RATIOS[1];
            accel *= (0.2 + 0.8 * gearFactor);
          }

          if (shiftTimerRef.current > 0) accel = 30000; // Snappier drop during shift
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-8 bg-[#1a1a1a] select-none">
      ...
      {!engineOn && rpm === 0 && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[200] backdrop-blur-md">
          ...
          >
            <h1 className="text-7xl font-black text-white mb-8 tracking-tighter uppercase italic">Dashboard (Alpha 2)</h1>
            <button 
      ...
      <div className="absolute top-12 left-1/2 -translate-x-1/2 text-center z-50 w-full">
        <h1 className="text-5xl md:text-6xl font-black tracking-tighter text-zinc-900 uppercase opacity-20 select-none pointer-events-none">Dashboard (Alpha 2)</h1>
      </div>
